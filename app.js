  ...
  return new Promise(resolve => {
    if (currentTargetMode === 'publik') {
      ...
      db.ref('rooms/' + room + '/chats').once('value').then(snap => {
        ...
        resolve(boleh); // Perbaikan: sebelumnya 'boh'
      });
    }
  });
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  notifInbox.onclick = () => {
    db.ref(`users/${ID}/chats`).once('value').then(async snapshot => {
      ...
      pengirimData.forEach(({ id, nama }) => {
        ...
        btn.onclick = () => {
          document.body.removeChild(container);
          window.location.href = `?id=${encodeURIComponent(id)}`; // SPA style
        };
  ...
  window.kirimKomentar = function(postId) {
    if (!ID) return alert("Silakan login dulu untuk komentar");
  ...
  window.toggleLike = function(postId) {
    if (!ID) return alert("Silakan login dulu untuk like");
    const refLike = db.ref("likes/" + postId + "/" + ID);
  ...
  window.tampilkanGambarPenuh = function(src) {
    ...
    const gambar = document.getElementById("
window.tampilkanGambarPenuh = function(src) {
    if (!src) return;
    const modal = document.getElementById("modalGambar");
