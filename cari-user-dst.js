...
}); window.kirimKomentar = function(postId) { if (!userID) return alert("Silakan login dulu untuk komentar"); const nama = localStorage.getItem("Nama") || "Anonim"; const input = document.getElementById("inputKomentar-" + postId); const teks = (input?.value || "").trim(); if (!teks) return; db.ref("komentar/" + postId).push({ ID: userID, Nama: nama, Teks: teks, Waktu: new Date().toISOString() }); input.value = ""; }; window.toggleLike = function(postId) { if (!userID) return alert("Silakan login dulu untuk like"); const refLike = db.ref("likes/" + postId + "/" + userID); refLike.once("value").then(snap => { if (snap.exists()) { refLike.remove(); } else { refLike.set(true); } }); }; window.tampilkanGambarPenuh = function(src) { if (!src) return; const modal = document.getElementById("modalGambar"); const gambar = document.getElementById("gambarModal"); gambar.src = src; modal.style.display = "flex"; }; document.getElementById("modalGambar").addEventListener("click", () => { document.getElementById("modalGambar").style.display = "none"; }); }); function setupModalSet(idx) { const modalPembayaran = document.getElementById("modalPembayaran" + idx); const tutupPembayaran = document.getElementById("tutupPembayaran" + idx); const produkEl = document.getElementById("produk" + idx); if (produkEl) { produkEl.addEventListener("click", e => { e.preventDefault(); modalPembayaran.style.display = "flex"; }); } tutupPembayaran.addEventListener("click", () => { modalPembayaran.style.display = "none"; }); modalPembayaran.addEventListener("click", e => { if (e.target === modalPembayaran) { modalPembayaran.style.display = "none"; } }); document.getElementById('btnQris' + idx).onclick = () => document.getElementById('modalQris' + idx).style.display = 'flex'; document.getElementById('btnDana' + idx).onclick = () => document.getElementById('modalDana' + idx).style.display = 'flex'; document.getElementById('btnOvo' + idx).onclick = () => document.getElementById('modalOvo' + idx).style.display = 'flex'; document.getElementById('btnGopay' + idx).onclick = () => document.getElementById('modalGopay' + idx).style.display = 'flex'; document.getElementById('btnShopeepay' + idx).onclick = () => document.getElementById('modalShopeepay' + idx).style.display = 'flex'; document.getElementById('tutupQris' + idx).onclick = () => document.getElementById('modalQris' + idx).style.display = 'none'; document.getElementById('tutupDana' + idx).onclick = () => document.getElementById('modalDana' + idx).style.display = 'none'; document.getElementById('tutupOvo' + idx).onclick = () => document.getElementById('modalOvo' + idx).style.display = 'none'; document.getElementById('tutupGopay' + idx).onclick = () => document.getElementById('modalGopay' + idx).style.display = 'none'; document.getElementById('tutupShopeepay' + idx).onclick = () => document.getElementById('modalShopeepay' + idx).style.display = 'none'; } setupModalSet(1); setupModalSet(2); setupModalSet(3); setupModalSet(4); 
...
document.getElementById('closeFaq').onclick = function() {
  document.getElementById('faqModal').style.display = 'none';
};
