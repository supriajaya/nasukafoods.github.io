// Memastikan variabel global dari app.js dapat diakses
...
function initializeHome() {
    ...
    // Event listener DOMContentLoaded tidak lagi dibutuhkan di sini.
    // Semua inisialisasi sekarang berada di dalam fungsi ini.
    ...
    // Event listener untuk menutup kwitansi
    ...
    // Expose fungsi yang dipanggil dari HTML
...
// --- Fungsi-fungsi dari file index.js Anda yang lama ---
...
function loadPosts() {
  ...
  db.ref("posts").orderByChild("Status").equalTo("publik").limitToLast(10).on("child_added", postSnap => {
    const post = postSnap.val();
    const postId = postSnap.key;

    const namaPemilik = post.Nama || "Tanpa Nama";
    const fotoPemilik = post.Foto || "https://nasukafoods.site/gambarkosong.jpg";
...
function tampilkanGambarPenuh(src) {
  if (!src) return;  
...
async function submitDigital() {
  ...
  if (kNama) kNama.textContent = localStorage.getItem("Nama");
  if (kTelepon) kTelepon.textContent = localStorage.getItem("Telepon");
  if (kAlamat) kAlamat.textContent = localStorage.getItem("Alamat");
...
pageInitializers['home'] = initializeHome;
